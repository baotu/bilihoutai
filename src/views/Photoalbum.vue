						...
						<el-tab-pane label="相册管理">
							<el-table style="width: 100%" :data="tables.slice((pageIndex-1)*pageSize,pageIndex*pageSize)">
								<el-table-column label="相册封面">
									<template slot-scope="scope">
										<el-popover placement="right" title="" trigger="hover">
											<img width="500" height="400" :src="scope.row.photo" />
											<img preview slot="reference" :src="scope.row.photo" :alt="scope.row.photo" style="max-height: 50px;max-width: 130px">
										</el-popover>
									</template>
								</el-table-column>
								<el-table-column :show-overflow-tooltip="true" label="相册名称" prop="albumname" ></el-table-column>
								<el-table-column :show-overflow-tooltip="true" label="用户名称" prop="xyuser.username"></el-table-column>
								<el-table-column :show-overflow-tooltip="true" label="创建时间" prop="time" sortable></el-table-column>
								<el-table-column align="right">
									...
									<template slot-scope="scope">
										<el-row>
											<el-button title="查看相册" size="mini" type="primary" @click="xp(scope.row)" icon="el-icon-picture" circle></el-button>
											<el-button title="相片审核" size="mini" type="success" @click="sh(scope.row)" icon="el-icon-check" circle></el-button>
											<el-button title="相册评论" size="mini" type="info" @click="pl(scope.row)" icon="el-icon-tickets" circle></el-button>
											<el-button title="删除相册" size="mini" type="danger" @click="handleDelete(scope.$index, scope.row)" icon="el-icon-delete" circle></el-button>
										</el-row>
									</template>
								</el-table-column>
							</el-table>
...
<script>
	export default {
		...
		methods: {
			...
			xp(row){
				this.$router.push({ path: 'photo', query : { albumid : row.albumid}});
			},
			sh(row){
				this.$router.push({ path: 'photosh', query : { albumid : row.albumid}});
			},
			pl(row){
				this.$router.push({ path: 'photopl', query : { albumid : row.albumid}});
			},
			handleDelete(index,row) {
				...
				this.$confirm('此操作将永久删除该相册, 是否继续?', '提示', {
					...
					this.$axios.get('xyalbum/deleteByAlbumidBack.do', {
						params: {
							albumid: row.albumid,
						}
					})
		...
		mounted() {
			...
			this.$axios.get('xyalbum/findAll.do')
			.then(rs => {
			...
			tables () {
				...
				if (search) {
					/* return this.tableData5.filter(
						data => !search || (data.albumname.toLowerCase().includes(search.toLowerCase()))
					) */
