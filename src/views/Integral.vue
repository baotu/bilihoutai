					...
					<el-tabs type="border-card">
						<el-tab-pane label="用户积分管理">
							<el-table :data="tables.slice((pageIndex-1)*pageSize,pageIndex*pageSize)" style="width: 100%">
								...
								<el-table-column label="总积分" prop="totalintegral" sortable> </el-table-column>
								<el-table-column align="right">
									<template slot="header" slot-scope="scope">
											<el-input v-model="search" size="mini" placeholder="输入关键字搜索"/>
									</template>
									<template slot-scope="scope">
										<el-input-number v-model="scope.row.totalintegral" size="mini" @change="handleChange(scope.row)" :min="0" :max="100000"></el-input-number>
									</template>
								</el-table-column>
							</el-table>
...
<script>
	export default {
		...
		methods: {
			...
			handleChange(row) {
				this.$axios.get('xyuser/updateIntegralByUserId.do', {
					params: {
						userid : row.userid,
						totalintegral : row.totalintegral
					}
				})
				.then(rs => {
					//console.log(rs.data)
