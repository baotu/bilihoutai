						...
						<el-tab-pane label="转账记录">
							<div style="">
								<el-table style="width: 100%" :data="tables.slice((pageIndex-1)*pageSize,pageIndex*pageSize)">
									<el-table-column label="转账编号" prop="transferid" sortable></el-table-column>
									<el-table-column label="转账用户" prop="xyuser.0.username"></el-table-column>
									<!-- <el-table-column label="转账类型" prop="transtype"></el-table-column> -->
									<el-table-column label="转账金额" prop="money" sortable></el-table-column>
									<el-table-column label="接收用户" prop="xyuser.1.username" ></el-table-column>
									<el-table-column label="交易状态">
										<template slot-scope="props">
											<span v-if="props.row.status == 1">交易成功</span>
											<span v-else>交易失败</span>
										</template>
									</el-table-column>
									<el-table-column :show-overflow-tooltip="true" label="交易时间" prop="time" sortable></el-table-column>
									<el-table-column :show-overflow-tooltip="true" label="备注" prop="remark"></el-table-column>
									<el-table-column align="right">
										<template slot="header" slot-scope="scope">
											<el-input v-model="search" size="mini" placeholder="输入用户搜索" />
										</template>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			this.$axios.get('xytransfer/findAll.do')
			.then(rs => {
			...
			tables () {
				...
				if (search) {
					return this.tableData5.filter(
						data => !search || (data.xyuser[0].username.toLowerCase().includes(search.toLowerCase()) || data.xyuser[1].username.toLowerCase().includes(search.toLowerCase()))
					)
				}
