			...
			<el-container>
				<el-main class="el-main">
					<el-tabs type="border-card">
						<el-tab-pane label="视频类型">
							<el-table style="width: 100%" :data="tables.slice((pageIndex-1)*pageSize,pageIndex*pageSize)">
								<el-table-column label="类型编号" prop="typeid" sortable></el-table-column>
	...
	export default {
		...
		methods: {
			handleCurrentChange(val) {
				this.page = val;
				this.$axios.get('xyvideotype/findPage.do', {
					params: {
						page: this.page,
				...
				.then(rs => {
					...
					this.totl = qwe;
					//console.log(rs.data.xyvideotype)
					this.tableData5 = rs.data.xyvideotype
				}).catch(err => {
					//console.log(err)
				});
			},
			...
			handleEdit(index, row) {
				var self = this;
				this.$prompt('请输入视频类型', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					inputValue : row.typename
				}).then(({ value }) => {
					this.$axios.get('xyvideotype/updateById.do', {
						params: {
							typeid : row.typeid,
							typename : value
						}
		...
		mounted() {
			...
			this.$axios.get('xyvideotype/findAll.do')
			.then(rs => {
				this.tableData5 = rs.data.xyvideotype;
			}).catch(err => {
				//console.log(err)
