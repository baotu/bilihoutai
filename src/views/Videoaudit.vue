				...
				<el-main class="el-main">
					<el-tabs type="border-card">
						<el-tab-pane label="已通过审核">
							<div style="">
								<el-table style="width: 100%" :data="tables.slice((pageIndex-1)*pageSize,pageIndex*pageSize)">
									<el-table-column label="视频ID" prop="videoid" width="150px" sortable></el-table-column>
									...
									<el-table-column label="审核状态" prop="checke">审核通过</el-table-column>
									<el-table-column align="right">
										<template slot="header" slot-scope="scope">
											<el-input v-model="search" size="mini" placeholder="输入关键字搜索" />
										</template>
										<template slot-scope="scope" width="200px">
											<el-button size="mini" @click="handleEdit1(scope.$index, scope.row)">取消审核通过</el-button>
										</template>
									</el-table-column>
								</el-table>
						...
						<el-tab-pane label="未通过审核">
							<el-table style="width: 100%" :data="table.slice((pageIndex1-1)*pageSize1,pageIndex1*pageSize1)">
								<el-table-column label="视频ID" prop="videoid" width="150px" sortable></el-table-column>
								<el-table-column :show-overflow-tooltip="true" label="视频名称" prop="videoname"></el-table-column>
								<el-table-column :show-overflow-tooltip="true" label="搜索标签" prop="label"></el-table-column>
								<el-table-column :show-overflow-tooltip="true" label="视频地址">
								...
								<el-table-column label="上传时间" prop="time" sortable></el-table-column>
								<el-table-column label="审核状态" prop="checke">审核未通过</el-table-column>
								<el-table-column align="right">
									<template slot="header" slot-scope="scope">
										<el-input v-model="search" size="mini" placeholder="输入关键字搜索" />
									</template>
									<template slot-scope="scope" width="200px">
										<el-button size="mini" @click="handleEdit(scope.$index, scope.row)">通过审核</el-button>
									</template>
								</el-table-column>
							</el-table>
...
<script>
	export default {
		...
		methods: {
			...
			handleEdit(index, row) {  //审核通过
				...
				this.$confirm('是否通过审核?', '提示', {
					...
					this.$axios.get('xyvideo/updateVideoPass.do', {
						params: {
							videoid : row.videoid
						}
					})
		...
		mounted() {
			...
			this.$axios.get('xyvideo/findPass.do')  //通过审核的视频
			.then(rs => {
			...
			this.$axios.get('xyvideo/findNotPass.do' )  //未通过审核的视频
			.then(rs => {
