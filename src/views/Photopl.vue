				...
				<el-main class="el-main">
					<el-tabs type="border-card">
						<el-breadcrumb separator="/">
							...
							<el-breadcrumb-item>相册评论</el-breadcrumb-item>
						</el-breadcrumb>
						<el-tab-pane label="相册评论">
							<el-table style="width: 100%" :data="tables.slice((pageIndex-1)*pageSize,pageIndex*pageSize)">
								<el-table-column label="头像">
									<template slot-scope="scope">
										<el-popover placement="right" title="" trigger="hover">
											<img width="300" height="200" :src="scope.row.xyuserinfo.userphoto" />
											<img slot="reference" :src="scope.row.xyuserinfo.userphoto" :alt="scope.row.xyuserinfo.userphoto" style="max-height: 50px;max-width: 50px;border-radius:50%">
										</el-popover>
										<!-- <img style="width: 50px;height: 50px;border-radius:50%" :src="scope.row.xyuserinfo.userphoto" /> -->
									</template>
								</el-table-column>
								<el-table-column label="评论人" prop="xyuser.username"></el-table-column>
								<el-table-column :show-overflow-tooltip="true" label="评论内容" prop="content"></el-table-column>
								<el-table-column label="评论时间" prop="time" sortable></el-table-column>
								<el-table-column align="right">
									<template slot="header" slot-scope="scope">
										<el-button size="mini" type="danger" @click="handleDelete1">删除所有评论</el-button>
									</template>
								</el-table-column>
								<el-table-column align="right">
									<template slot="header" slot-scope="scope">
										<el-input width="100" v-model="search" size="mini" placeholder="输入关键字搜索" />
									</template>
	...
	export default {
		...
		methods: {
			...
			handleDelete(index, row) {
				//console.log(row.xyuserinfo.userphoto)
				var self = this;
				this.$confirm('此操作将删除该评论, 是否继续?', '提示', {
					...
					this.$axios.get('xypgotocomment/deleteByCommentIdBack.do', {
						params: {
			...
			handleDelete1(){
				var self = this;
				this.$confirm('此操作将删除所有评论, 是否继续?', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning',
					center: true
				}).then(() => {
					this.$axios.get('xypgotocomment/deleteById.do', {
						params: {
							photoid: this.$route.query.albumid,
						}
					})
					.then(rs => {
						this.$message({
		...
		mounted() {
			...
			this.$axios.get('xypgotocomment/findByPhotoId.do',{
				params:{
					photoid : this.$route.query.albumid,
				}
			})
			...
			tables () {
			  ...
			  if (search) {
				return this.tableData5.filter(
				  	data => !search || (data.xyuser.username.toLowerCase().includes(search.toLowerCase()) || data.content.toLowerCase().includes(search.toLowerCase()))
				)
