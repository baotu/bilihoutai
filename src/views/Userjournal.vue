					...
					<el-tabs type="border-card">
						<el-tab-pane label="日志记录">
							<el-table style="width: 100%" :data="tables.slice((pageIndex-1)*pageSize,pageIndex*pageSize)">
								<el-table-column label="日志编号" prop="logid" sortable></el-table-column>
								<el-table-column label="用户编号" prop="userid"></el-table-column>
								<el-table-column label="登录状态" prop="remark" sortable></el-table-column>
								<el-table-column label="日志创建时间" prop="time" ></el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			this.$axios.get('xylog/findAllByTime.do')
			.then(rs => {
